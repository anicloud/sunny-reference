...
angular.module('pascalprecht.translate').provider('$translate', ['$STORAGE_KEY', function ($STORAGE_KEY) {
    ...
    var getLocale = function () {
        ...
        //Ching-zhou modified
        var curLangArr = (getFirstBrowserLanguage() || '').split('-');
        if(curLangArr.length > 1 && !$translationTable[curLangArr[0] + '_' + curLangArr[1].toUpperCase()]){
            curLangArr = curLangArr.splice(0, 1);
        }
        if(curLangArr.length < 2 && !$translationTable[curLangArr[0]]){
            return 'en';
        }
        return curLangArr.join('_');
    };
